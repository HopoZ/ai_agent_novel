...
| `App.vue` | 根布局：左栏上下文 + 中栏导演工作台，运行结果改为抽屉；含影子编导自动细节接管（可撤销）、一致性审计阻断续写、结构卡风险确认，以及“自动设定管理”（查看/重生成创建时自动 lores 草案）和 Tag 内置管理（单条 + 批量删除/批量前缀迁移）；下章建议改为右栏内联编辑与一键续写 |
...
| `components/MidFormPanel.vue` | 中栏导演工作台：Step1~Step4 手动流转；阶段完成后高亮下一步；非当前步骤默认隐藏；Step3 任务为空时自动推测草案并可一键重置 |
