...
from .storage import get_state_path, ensure_novel_dirs, load_chapter, load_state, save_chapter, save_state
...
class NovelAgent:
    ...
    def _lorebook(self, lore_tags: Optional[list[str]] = None) -> str:
        if lore_tags:
            lore = self.lore_loader.get_lore_by_tags(lore_tags)
        else:
    ...
    def init_state(self, novel_id: str, user_task: str, lore_tags: Optional[list[str]] = None) -> NovelState:
        state = load_state(novel_id)
        if not state:
            raise ValueError(f"novel_id not found: {novel_id}")

        lorebook = self._lorebook(lore_tags)
        state_context = self._format_state_for_prompt(state)
        ...
        plan_json = self._invoke_json(system, human, root_model=NovelState)
    ...
    def plan_chapter(
        ...
        continuity_hint = {
            ...
            "pov_character_id_override": pov_character_id_override,
        }
        ...
        human = (
            ...
            "- pov_character_id：若覆盖提供就使用，否则选择最稳定的 POV\n"
            "- who_is_present：列出在本章关键行动中出现的主要角色\n"
    ...
    def merge_state(base: NovelState, patch: NovelState) -> NovelState:
        ...
        # meta
        merged.meta = patch.meta or merged.meta
        if not merged.meta.novel_id:
            merged.meta.novel_id = base.meta.novel_id
        if not merged.meta.novel_title:
            merged.meta.novel_title = base.meta.novel_title
        if not merged.meta.lore_tags:
            merged.meta.lore_tags = base.meta.lore_tags
    ...
    def run(
        ...
        if mode == "init_state":
            new_state = self.init_state(novel_id, user_task=user_task, lore_tags=lore_tags)
        ...
        if mode in {"plan_only", "write_chapter", "revise_chapter"}:
            if not state.meta.initialized:
                ...
                self.init_state(novel_id, user_task=auto_task, lore_tags=lore_tags)
    ...
    def _invoke_json(self, system: str, human: str, root_model):
        """
